	...
	for (fastgltf::Material& mat : gltf.materials) {
		...
		if (mat.pbrData.baseColorTexture.has_value()) {
			...
			newMat->debug_sets.base_color_set =
				ImGui_ImplVulkan_AddTexture(file.samplers[sampler], timg.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
		}
		...
		if (mat.pbrData.metallicRoughnessTexture.has_value()) {
			...
			newMat->debug_sets.metal_roughness_set =
				ImGui_ImplVulkan_AddTexture(file.samplers[sampler], timg.view,
...
std::optional<std::pair<ImageID, std::string>> loadImage(
	...
	const auto mime_to_str = [](fastgltf::MimeType mime) {
		...
		case fastgltf::MimeType::GltfBuffer: return "GltfBuffer";
		case fastgltf::MimeType::JPEG: return "JPEG";
		case fastgltf::MimeType::KTX2: return "KTX2";
		case fastgltf::MimeType::None: return "NONE";
		case fastgltf::MimeType::OctetStream: return "OctetStream";
	...
	std::visit(
		fastgltf::visitor{
			...
			[&](fastgltf::sources::URI& filePath) {
			  ...
			  assert(
				  filePath.uri.isLocalPath());  // We're only capable of loading
			  // local files.
			  ...
			  if (data) {
				...
				image_id = engine->image_codex.loadImageFromFile(path, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT, true);
			...
			[&](fastgltf::sources::Vector& vector) {
			  ...
			  if (data) {
				...
				image_id = engine->image_codex.loadImageFromData("hello.test", data, imagesize, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT, true);
			...
			[&](fastgltf::sources::BufferView& view) {
			  ...
			  std::visit(fastgltf::visitor{
				  ...
				  [&](fastgltf::sources::Vector& vector) {
					...
					std::string debug_name = std::format("{}.{}", view.bufferViewIndex, mime_to_str(view.mimeType));
					image_id = engine->image_codex.loadImageFromData(debug_name.c_str(), data, imagesize, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT, true);
