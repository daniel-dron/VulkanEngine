...
namespace ImGuiUtils
{
  ...
  public:
    ...
    ProfilerGraph(size_t framesCount) : frames( framesCount )
    {
      frameWidth = 3;
      frameSpacing = 1;
    ...
    void LoadFrameData(const legit::ProfilerTask *tasks, size_t count)
    {
      ...
      {
        if (taskIndex == 0)
          currFrame.push_back(tasks[taskIndex]);
        else
        {
          ...
          {
            currFrame.back().endTime = tasks[taskIndex].endTime;
          }
        }
      }

      for (size_t taskIndex = 0; taskIndex < currFrame.size(); taskIndex++)
      {
        auto &task = currFrame[taskIndex];
  ...
  private:
    void RebuildTaskStats(size_t endFrame, size_t framesCount)
    {
      ...
      for (size_t frameNumber = 0; frameNumber < framesCount; frameNumber++)
      {
        ...
        for (size_t taskIndex = 0; taskIndex < frame.size(); taskIndex++)
        {
          auto &task = frame[taskIndex];
        auto &stats = taskStats[taskNameToStatsIndex[task.name]];
    ...
    void RenderGraph(ImDrawList *drawList, glm::vec2 graphPos, glm::vec2 graphSize, size_t frameIndexOffset)
    {
        ...
        {
            size_t frameIndex = (frames.Size() - frameIndexOffset - 1 - frameNumber + 2 * frames.Size()) % frames.Size();
    ...
    void RenderLegend(ImDrawList *drawList, glm::vec2 legendPos, glm::vec2 legendSize, size_t frameIndexOffset)
    {
      ...
      {
        auto &task = currFrame[taskIndex];
        auto &stat = taskStats[taskNameToStatsIndex[task.name]];
    ...
    CircularQueue<std::vector<legit::ProfilerTask>> frames;
