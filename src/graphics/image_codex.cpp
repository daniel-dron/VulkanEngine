...
ImageId ImageCodex::LoadHdrFromFile( const std::string &path, const VkFormat format, const VkImageUsageFlags usage, const bool mipmapped ) {
    // search for already loaded image
    for ( const auto &img : m_images ) {
        if ( path == img.GetName( ) && format == img.GetFormat( ) && usage == img.GetUsage( ) &&
             mipmapped == img.IsMipmapped( ) ) {
...
ImageId ImageCodex::LoadCubemapFromFile( const std::vector<std::string> &paths, const VkFormat format, const VkImageUsageFlags usage, const bool mipmapped ) {
...
ImageId ImageCodex::LoadCubemapFromData( const std::vector<std::string> &paths, const std::vector<unsigned char *> &datas, const VkExtent3D extent, const VkFormat format, const VkImageUsageFlags usage, const bool mipmapped ) {
    ...
    GpuImage image = GpuImage( m_gfx, paths.at( 0 ), ( void * )merged_data, extent, format, ImageType::TCubeMap, usage, mipmapped );
...
ImageId ImageCodex::CreateCubemap( const std::string &name, const VkExtent3D extent, const VkFormat format, const VkImageUsageFlags usage, int mipmaps ) {
...
ImageId ImageCodex::CreateEmptyImage( const std::string &name, const VkExtent3D extent, const VkFormat format, const VkImageUsageFlags usage, const bool mipmapped ) {
...
ImageId ImageCodex::LoadImageFromData( const std::string &name, void *data, const VkExtent3D extent, const VkFormat format, const VkImageUsageFlags usage, const bool mipmapped ) {
...
VkDescriptorSetLayout ImageCodex::GetBindlessLayout( ) const {
    return bindlessRegistry.layout;
}

VkDescriptorSet ImageCodex::GetBindlessSet( ) const {
    return bindlessRegistry.set;
}
...
void ImageCodex::DrawDebug( ) const {
    ImGui::Columns( 10 );
    {
        for ( int i = 1; i < m_images.size( ); i++ ) {
            ...
            ImGui::Image( reinterpret_cast<ImTextureID>( static_cast<ImageId>( i ) ), ImVec2( column_width, column_width ) );
            if ( ImGui::IsItemHovered( ) ) {
                ImGui::BeginTooltip( );
                ImGui::Text( "%s", image.GetName( ).c_str( ) );
                ImGui::Separator( );
                ImGui::Image( reinterpret_cast<ImTextureID>( static_cast<ImageId>( i ) ), ImVec2( image.GetExtent( ).width, image.GetExtent( ).height ) );
                ImGui::EndTooltip( );
            }
            ImGui::NextColumn( );
        }
...
void ImageCodex::InitDefaultImages( ) {
    ...
    m_checkboard = LoadImageFromData( "debug_checkboard_img", ( void * )&white, VkExtent3D{ 16, 16, 1 }, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT, false );
}
